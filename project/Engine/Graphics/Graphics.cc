...
Graphics::Graphics(SDL_Window* window) : window{window}
{
  ...
  SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "linear");
    ...
    font = TTF_OpenFont("./Fonts/slkscrb.ttf", 20);
    if(!font)
    {
      cerr << TTF_WasInit() << endl;
...
Graphics::~Graphics()
{
  ...
  SDL_DestroyTexture(rendered_image);
...
void Graphics::draw(vector<Sprite*>& sprite_list, int camera_x, int camera_y)
{
  ...
  // Adjust the players x and y position
  ...
  // Render everything to rendered_image

  // Draw background
  drawBackground(screen_width, screen_height, camera_x, camera_y, draw_offset);

  // Draw Sprites
  drawSprites(sprite_list, camera_x, camera_y, draw_offset);

  // Set opacity and positional options on rendered_image
  SDL_Rect screen_rect{0, 0, screen_width, screen_height};
  SDL_SetTextureBlendMode(rendered_image, SDL_BLENDMODE_BLEND);
  SDL_SetTextureAlphaMod(rendered_image, motion_blur);

  // Render low opacity copy of rendered_image to screen_texture
  SDL_SetRenderTarget(renderer, screen_texture);
  SDL_RenderCopy(renderer, rendered_image, nullptr, &screen_rect);

  // Render screen_texture to default target
...
void Graphics::drawBackground(int screen_width, int screen_height, int camera_x, int camera_y, SDL_Point draw_offset)
{
  // mod_camera_x is how many pixels of the background that 
  // need to be rendered to the left of the window
  int mod_camera_x = camera_x % screen_width;
  mod_camera_x += screen_width;
  mod_camera_x %= screen_width;
  mod_camera_x = screen_width-mod_camera_x;
  // mod_camera_y is how many pixels of the background 
  // that need to be rendered to above the window
...
void Graphics::drawSprites(vector<Sprite*>& sprite_list, int camera_x, int camera_y, SDL_Point draw_offset)
{
  ...
  for(Sprite* sprite : sprite_list)
  {
    ...
    // Set render destination according to camera position and draw offset
